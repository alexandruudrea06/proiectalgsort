...
class SortingAlgorithms:
    ...
    def quick_sort(arr):
        ...
        return (
            SortingAlgorithms.quick_sort(left)
            + middle
...
def generate_data(size, data_type="random"):
    if data_type == "random":
        ...
        return (
            list(range(half))
            + [random.randint(half, size * 2) for _ in range(size - half)]
        )
...
def run_medium_arrays():
    print_section(
        "MEDIUM ARRAYS (random data) - ALL ALGORITHMS (including O(n²) for 50,000)"
    )
...
def run_massive_arrays():
    ...
    print("    - 1B:         Timsort only (pure Python implementations would take hours)")
    print()
    ...
    for algo_name in algorithms_full:
        ...
        for size in sizes_full:
            print(
                f"  Running {algo_name} on {size:,} elements...",
                end="",
                flush=True
            )
    ...
    print(
        "  Running Timsort on 1,000,000,000 elements...",
        end="",
        flush=True
    )
